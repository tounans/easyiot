        ...
        while true do
            ...
            if socket.isReady() then
                ...
                if mqttClient:connect("s5.nsloop.com",25910,"tcp") then
                    ...
                    -- 请求主题
                    local requestTopid=string.format("/request/%s", imei)
                    if mqttClient:subscribe(requestTopid) then

                        -- if mqttClient:publish(reportTopid,json.encode({code="N1100",data=net.getCellInfoExt()})) then
