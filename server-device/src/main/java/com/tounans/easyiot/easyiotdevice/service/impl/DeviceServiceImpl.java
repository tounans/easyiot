...
public class DeviceServiceImpl extends ServiceImpl<DeviceMapper, Device> implements IDeviceService {
    ...
    @Override
    public Device getByUserAndImei(Integer userId, String imei) {
        return getOne(new QueryWrapper<Device>().lambda().eq(Device::getImei,imei).eq(Device::getUserId,userId));
    }

    @Override
    public IPage<Device> pageByUserParam(Integer page,Integer pageSize, String imei, Integer userId) {
        IPage<Device> devicePage = new Page<Device>().setCurrent(page).setSize(pageSize).addOrder(OrderItem.asc("user_device_id"));
